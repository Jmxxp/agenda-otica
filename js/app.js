...
class App {
    ...
    init() {
        ...
        if (this.initialized) {
            console.log('🔄 App já inicializado, fazendo refresh');
            this.refresh();
            return;
        }
    ...
    refresh() {
        ...
        if (sheetsDB?.isConnected) {
            sheetsDB.loadAll().then(() => {
                this.renderCalendar();
                this.renderSchedule();
            });
        } else {
    ...
    bindEvents() {
        try {
            ...
            // Clear all button
            document.getElementById('btn-clear-all').addEventListener('click', () => this.clearAllAppointments());
            ...
            console.log('✅ Todos os eventos ligados!');
        } catch (error) {
            console.error('❌ Erro ao ligar eventos:', error);
        }
    ...
    // Buscar agendamentos para uma data
    getAppointmentsForDate(dateStr) {
        // USA PLANILHA COMO FONTE ÚNICA
        if (sheetsDB?.isConnected) {
            return sheetsDB.getAppointments(dateStr);
        }
        return [];
    }
    ...
    bindScheduleEvents() {
        ...
        document.querySelectorAll('.simplified-cell.empty').forEach(cell => {
            cell.addEventListener('click', () => {
                ...
                if (!sheetsDB?.isConnected) {
                    this.showToast('Conecte à planilha primeiro! Clique em CALENDAR', 'error');
                    return;
                }
                ...
                if (loggedStoreId && storeId !== loggedStoreId) {
                    this.showToast('Você só pode agendar na sua própria loja', 'error');
                    return;
                }
    ...
    showAppointmentDetails(aptId) {
        // Buscar agendamento na planilha
        const apt = sheetsDB?.isConnected ? sheetsDB.getAppointment(aptId) : null;        
        if (!apt) return;
    ...
    async handleAppointmentSubmit(e) {
        ...
        // Verificar permissão
        if (id && loggedStoreId) {
            const existingApt = sheetsDB?.getAppointment(parseInt(id));
        ...
        // VERIFICAR SE ESTÁ CONECTADO
        if (!sheetsDB?.isConnected) {
            this.showToast('Conecte à planilha primeiro!', 'error');
            return;
        }
        ...
        if (id) {
            // EDITAR NA PLANILHA
            const result = await sheetsDB.updateAppointment(parseInt(id), aptData);
            if (result.success) {
            ...
            // CRIAR NA PLANILHA
            const result = await sheetsDB.createAppointment(aptData);
            if (result.success) {
    ...
    async deleteAppointment() {
        ...
        // Verificar permissão
        if (loggedStoreId) {
            const apt = sheetsDB.getAppointment(parseInt(id));
        ...
        if (confirm('Excluir este agendamento?')) {
            ...
            // Deletar da planilha
            const result = await sheetsDB.deleteAppointment(parseInt(id));
    ...
    async clearAllAppointments() {
        ...
        if (!confirm('⚠️ ATENÇÃO!\n\nIsso vai EXCLUIR TODOS os agendamentos!\n\nTem certeza?')) {
            return;
        }
        
        if (!confirm('🚨 CONFIRMAÇÃO FINAL!\n\nTodos os agendamentos serão apagados permanentemente.\n\nDeseja continuar?')) {
            return;
        }
        
        this.showToast('Limpando...', 'info');
        
        // Limpar na planilha
        const result = await sheetsDB.clearAll();
        
        if (result.success) {
            this.showToast('Todos os agendamentos foram excluídos!', 'success');
        } else {
            this.showToast('Erro: ' + result.error, 'error');
        }
    ...
    openCalendarModal() {
        // Update sheets UI state before showing
        if (typeof sheetsDB !== 'undefined' && sheetsDB) {
            sheetsDB.updateUI();
        }
    ...
    showToast(message, type = 'info') {
        ...
        // Tentar usar o authManager
        if (typeof authManager !== 'undefined' && authManager && authManager.showToast) {
