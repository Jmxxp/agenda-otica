...
const App = {
    ...
    TIMES.forEach(time => {
      ...
      STORES.forEach(s => {
        ...
        if (apt) {
          ...
          html += `
            <div class="grid-cell${canAdd ? '' : ''}" data-store="${s.id}" data-time="${time}">
              ${canAdd ? '<button class="add-btn" title="Agendar">+</button>' : ''}
            </div>`;
        }
      });
    });
  ...
  bindEvents() {
    ...
    // Sidebar nav
    document.getElementById('nav-config').addEventListener('click', () => this.openConfig());
    document.getElementById('btn-logout').addEventListener('click', () => this.logout());

    // Debug console
    document.getElementById('btn-console')?.addEventListener('click', () => this.toggleConsole());
    document.getElementById('debug-clear')?.addEventListener('click', () => this.clearConsole());
  },
