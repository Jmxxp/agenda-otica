...
class App {
    ...
    renderGeralSchedule() {
        ...
        const appointments = dataService.getAppointments(dateStr);
                ...
                <div class="simplified-row" style="grid-template-columns: ${gridCols}">
                    ...
                    ${stores.map(store => {
                        ...
                        if (apt) {
                            ...
                            // Another store has an appointment at this time - block this cell
                            return `
    ...
    renderSimplificadoSchedule() {
        ...
        if (isNormalDay) {
            ...
            // Morning row
            html += this.renderSimplificadoPeriod('Manhã', morningSlots, appointments, 'sun');
            
            // Afternoon row
            html += this.renderSimplificadoPeriod('Tarde', afternoonSlots, appointments, 'cloud-sun');
        } else if (dayOfWeek === 5) {
            // Friday - afternoon only
            html += this.renderSimplificadoPeriod('Tarde', timeSlots, appointments, 'cloud-sun');
        } else if (dayOfWeek === 6) {
            // Saturday - morning only
            html += this.renderSimplificadoPeriod('Manhã', timeSlots, appointments, 'sun');
        } else {
            // Fallback
            html += this.renderSimplificadoPeriod('Horários', timeSlots, appointments, 'clock');
        }
    ...
    renderSimplificadoPeriod(label, timeSlots, appointments, icon = 'clock') {
