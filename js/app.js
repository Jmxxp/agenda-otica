...
const App = {
    ...
    times.forEach(time => {
      ...
      STORES.forEach(s => {
        ...
        if (apt) {
          ...
          // Pode agendar? Só na própria loja (nunca em viewer)
          const canAdd = !this.viewerMode && s.id === this.store.id;
          html += `
  ...
  renderSimplified() {
    ...
    const storesToShow = isViewer ? STORES : [store];
    ...
    sections.forEach(sec => {
      ...
      storesToShow.forEach(st => {
        ...
        sec.times.forEach(time => {
          const apt = apts.find(a => a.time === time && a.storeId === st.id);
          if (apt) {
            const phone = this.fmtPhone(apt.phone);
            html += `
              <td class="simp-td simp-filled" data-apt-id="${apt.id}">
                <div class="simp-card store-${st.id}">
                  <div class="simp-card-store">${st.name}</div>
            ...
            html += `
              <td class="simp-td simp-empty" data-store="${st.id}" data-time="${time}">
