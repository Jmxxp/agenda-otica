...
class App {
    ...
    bindEvents() {
        ...
        document.getElementById('btn-google-connect')?.addEventListener('click', () => {
            if (calendarService) {
                ...
                this.showToast('Google API não carregada', 'error');
            }
        });
        ...
        document.getElementById('calendar-select')?.addEventListener('change', (e) => {
            if (calendarService) {
                calendarService.selectCalendar(e.target.value);
            }
        });

        document.getElementById('btn-sync-to-google')?.addEventListener('click', () => {
            if (calendarService) {
                calendarService.syncToGoogle();
            }
        });

        document.getElementById('btn-sync-from-google')?.addEventListener('click', () => {
            if (calendarService) {
                calendarService.syncFromGoogle().then(imported => {
                    if (imported > 0) {
                        this.refresh();
                        this.showToast(`${imported} eventos importados!`, 'success');
                    } else {
                        this.showToast('Nenhum evento novo encontrado', 'info');
                    }
                }).catch(() => {
                    this.showToast('Erro ao importar eventos', 'error');
                });
            }
        });
    ...
    // Combinar agendamentos locais + Google Calendar (Calendar tem prioridade)
    getAppointmentsForDate(dateStr) {
        let appointments = [];
        
        // PRIORIDADE 1: Dados do Google Calendar (sincronizados entre dispositivos)
        if (typeof calendarService !== 'undefined' && calendarService && 
            calendarService.isConnected && calendarService.selectedCalendarId) {
            appointments = calendarService.getAppointmentsFromCalendar(dateStr);
        }
        
        // PRIORIDADE 2: Dados locais (fallback se Calendar não conectado)
        if (appointments.length === 0) {
            appointments = dataService.getAppointments(dateStr);
        }
    ...
    renderSimplificadoPeriod(label, timeSlots, appointments, icon = 'clock', dateStr = null) {
        ...
        timeSlots.forEach(time => {
            ...
            // Check if Google Calendar has an event at this time
            const googleBusy = dateStr && typeof calendarService !== 'undefined' && calendarService && 
                              calendarService.isTimeSlotBusy(dateStr, time);
            ...
            if (apt) {
                ...
                // Blocked by Google Calendar event
                html += `
                    <div class="google-event-block">
                        <span>📅</span>
                        <small>Ocupado</small>
                    </div>
                `;
    ...
    showAppointmentDetails(aptId) {
        // Buscar agendamento: primeiro no Calendar, depois local
        let apt = null;
        let fromCalendar = false;
        
        // Tentar buscar do Calendar primeiro
        if (typeof calendarService !== 'undefined' && calendarService && calendarService.isConnected) {
            const calendarApts = calendarService.getAppointmentsFromCalendar();
            apt = calendarApts.find(a => a.id === aptId || a.googleEventId === aptId);
            if (apt) fromCalendar = true;
        }
        
        // Fallback para dados locais
        if (!apt) {
            apt = dataService.getAppointment(parseInt(aptId));
        }
        
        if (!apt) return;

        const store = apt.fromCalendar 
            ? { name: apt.storeName, color: apt.storeColor }
            : dataService.getStore(apt.storeId);
    ...
    handleAppointmentSubmit(e) {
        ...
        // Verificar se é um evento do Calendar (googleEventId é uma string longa)
        const isCalendarEvent = id && (id.length > 10 || isNaN(parseInt(id)));
        
        // Get store from form or use logged-in store
        let storeId = parseInt(document.getElementById('apt-store').value);
        if (loggedStoreId && !id) {
            // Force logged-in store for new appointments
        ...
        // Se é evento do Calendar, atualizar diretamente no Calendar
        if (isCalendarEvent && typeof calendarService !== 'undefined' && calendarService && calendarService.isConnected) {
            data.id = 0; // ID temporário
            data.googleEventId = id; // Usar googleEventId para atualizar
            
            calendarService.createOrUpdateEvent(data).then(async (success) => {
                if (success) {
                    this.showToast('Agendamento atualizado!', 'success');
                    await calendarService.loadGoogleEvents();
                    this.closeAllModals();
                    this.renderSchedule();
                    this.renderCalendar();
                }
        ...
        // Check permission for edit (local)
        if (id && loggedStoreId && !isCalendarEvent) {
            const existingApt = dataService.getAppointment(parseInt(id));
            if (existingApt && existingApt.storeId !== loggedStoreId) {
                this.showToast('Você não tem permissão para editar este agendamento', 'error');
                return;
            }
        }

        let result;
        if (id && !isCalendarEvent) {
            result = dataService.updateAppointment(parseInt(id), data);
        } else {
            result = dataService.addAppointment(data);
        }

        if (result.success) {
            this.showToast(id ? 'Agendamento atualizado!' : 'Agendamento criado!', 'success');
            this.closeAllModals();
            this.renderSchedule();
            this.renderCalendar();
            this.loadStats();
            
            // AUTO-SYNC: Sincronizar com Google Calendar automaticamente
            if (typeof calendarService !== 'undefined' && calendarService && calendarService.isConnected && calendarService.selectedCalendarId) {
                const apt = result.appointment || dataService.getAppointment(parseInt(id));
                if (apt) {
                    const calendarName = calendarService.calendars?.find(c => c.id === calendarService.selectedCalendarId)?.summary || 'Google Calendar';
                    this.showToast(`Sincronizando com "${calendarName}"...`, 'info');
                    
                    calendarService.syncSingleAppointment(apt).then((success) => {
                        if (success) {
                            console.log('Auto-sync to Google Calendar completed');
                            this.showToast(`✅ Evento criado em "${calendarName}" para ${apt.date} às ${apt.time}`, 'success');
                        } else {
                            console.warn('Auto-sync returned false');
                            this.showToast('⚠️ Agendamento salvo localmente, mas NÃO foi para o Google Calendar', 'error');
                        }
                    }).catch(err => {
                        console.error('Auto-sync error:', err);
                        this.showToast('⚠️ Erro ao sincronizar com Google Calendar: ' + (err.message || err), 'error');
                    });
                }
            } else if (typeof calendarService !== 'undefined' && calendarService && calendarService.isConnected && !calendarService.selectedCalendarId) {
                // Conectado mas sem calendário selecionado
                this.showToast('⚠️ Selecione um calendário nas Configurações para sincronizar', 'warning');
            } else {
                this.showToast('ℹ️ Salvo apenas localmente (Google Calendar não conectado)', 'info');
            }
        } else {
    ...
    deleteAppointment() {
        ...
        // Verificar se é um evento do Calendar
        const isCalendarEvent = id && (id.length > 10 || isNaN(parseInt(id)));
        
        // Se é evento do Calendar, deletar diretamente
        if (isCalendarEvent && typeof calendarService !== 'undefined' && calendarService && calendarService.isConnected) {
            if (confirm('Tem certeza que deseja excluir este agendamento?')) {
                // Deletar diretamente do Calendar usando o googleEventId
                gapi.client.calendar.events.delete({
                    calendarId: calendarService.selectedCalendarId,
                    eventId: id
                }).then(async () => {
                    ...
                    console.error('Error deleting from Calendar:', err);
                    this.showToast('Erro ao excluir do Calendar', 'error');
                });
            }
            return;
        }
        ...
        if (loggedStoreId) {
            ...
            if (apt && apt.storeId !== loggedStoreId) {
                this.showToast('Você não tem permissão para excluir este agendamento', 'error');
                return;
            }
        }

        if (confirm('Tem certeza que deseja excluir este agendamento?')) {
            // Pegar agendamento antes de excluir para deletar do Google
            const apt = dataService.getAppointment(parseInt(id));
            
            // AUTO-SYNC: Deletar do Google Calendar
            if (apt && typeof calendarService !== 'undefined' && calendarService && calendarService.isConnected && calendarService.selectedCalendarId) {
                calendarService.deleteEvent(apt).then(() => {
    ...
    openCalendarModal() {
        // Update calendar service UI state before showing
        if (calendarService) {
            calendarService.updateUI();
        }
