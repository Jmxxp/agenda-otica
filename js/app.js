...
class App {
    ...
    refresh() {
        ...
        if (typeof sheetsService !== 'undefined' && sheetsService && sheetsService.isConnected) {
            sheetsService.autoSyncFromSheets().then(() => {
                this.renderCalendar();
                this.renderSchedule();
            });
        } else {
    ...
    handleAppointmentSubmit(e) {
        ...
        if (result.success) {
            ...
            // AUTO-SYNC: Sincronizar com Google Sheets automaticamente
            if (typeof sheetsService !== 'undefined' && sheetsService && sheetsService.isConnected) {
                ...
                if (apt) {
                    ...
                    console.log('📊 Sincronizando com planilha:', aptToSync);
                    
                    if (id) {
                        sheetsService.updateAppointment(apt.id, aptToSync).then(res => {
                            console.log('📊 Update result:', res);
                        }).catch(err => {
                            console.error('📊 Update error:', err);
                        });
                    } else {
                        sheetsService.createAppointment(aptToSync).then(res => {
                    ...
                    console.error('📊 Agendamento não encontrado para sync');
                }
            } else {
                console.log('📊 Sheets não conectado, salvando apenas local');
            }
        } else {
            this.showToast(result.error, 'error');
        }
    ...
    deleteAppointment() {
        ...
        if (confirm('Tem certeza que deseja excluir este agendamento?')) {
            // Pegar agendamento antes de excluir
            const apt = dataService.getAppointment(parseInt(id));
            
            // AUTO-SYNC: Deletar da planilha Google Sheets
            if (apt && typeof sheetsService !== 'undefined' && sheetsService && sheetsService.isConnected) {
                sheetsService.deleteAppointment(apt.id).catch(err => {
