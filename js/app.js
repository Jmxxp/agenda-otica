...
const App = {
  ...
  openNew(storeId, time) {
    ...
    document.getElementById('f-wpp').addEventListener('click', () => {
      ...
      const dateDisplay = document.getElementById('f-date').value;
      if (phone) {
        const msg = encodeURIComponent(`Olá ${client || 'cliente'}! Confirmamos seu agendamento para o dia ${dateDisplay} às ${time}. Aguardamos você!`);
  ...
  openDetail(id) {
    ...
    document.getElementById('d-wpp')?.addEventListener('click', () => {
      ...
      if (phone) {
        ...
        const msg = encodeURIComponent(`Olá ${name}! Confirmamos seu agendamento para o dia ${dateDisplay} às ${time}. Aguardamos você!`);
        window.open(`https://wa.me/55${phone}?text=${msg}`, '_blank');
      }
    });
