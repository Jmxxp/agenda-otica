...
class SheetsService {
    constructor() {
        ...
        this.lastSync = null;
        this.isSyncing = false; // Lock para evitar sync durante operações
        this.loadSettings();
    }
    ...
    async createAppointment(appointment) {
        ...
        // Bloquear sync enquanto cria
        this.isSyncing = true;
        console.log('📊 Bloqueando sync para criar...');
        ...
        try {
            ...
            const text = await response.text();
            console.log('📊 Resposta do create:', text);
            const data = JSON.parse(text);
            ...
            if (data.success) {
                ...
                console.error('📊 Erro no create:', data.error);
            }
            
            // Aguardar um pouco antes de liberar o lock
            setTimeout(() => {
                this.isSyncing = false;
                console.log('📊 Sync desbloqueado');
            }, 3000);
    ...
    // Sync silencioso (sem toasts) - FAZ MERGE, NÃO SUBSTITUI
    async autoSyncFromSheets() {
        if (!this.isConnected) return;
        
        // Não rodar se está no meio de uma operação
        if (this.isSyncing) {
            console.log('📊 Sync bloqueado, aguardando...');
            return;
        }

        try {
            console.log('📊 Buscando dados da planilha...');
            const remoteAppointments = await this.getAppointments();
            console.log('📊 Recebido da planilha:', remoteAppointments.length, 'agendamentos');
            
            const db = new Database();
            const data = db.getData();
            const localAppointments = data.appointments || [];
            
            console.log('📊 Dados locais:', localAppointments.length, 'agendamentos');
            
            // PROTEÇÃO: Se planilha vazia mas tem dados locais, NÃO apagar!
            // Apenas tentar enviar os dados locais para a planilha
            if (remoteAppointments.length === 0 && localAppointments.length > 0) {
                console.log('📊 Planilha vazia mas tem dados locais - enviando para planilha...');
                
                for (const apt of localAppointments) {
                    try {
                        const store = typeof dataService !== 'undefined' ? dataService.getStore(apt.storeId) : null;
                        const aptToSync = {
                            ...apt,
                            storeName: store ? store.name : 'Loja ' + apt.storeId,
                            clientPhone: String(apt.clientPhone || '')
                        };
                        await this.createAppointmentSilent(aptToSync);
                        console.log('📊 Enviado:', apt.clientName);
                    } catch (e) {
                        console.error('📊 Erro ao enviar:', e);
                    }
                }
                
                // NÃO alterar dados locais, apenas marcar sync
                this.lastSync = new Date().toISOString();
                this.saveSettings();
                this.updateUI();
                return; // Sair sem modificar dados locais
            }
            
            // Criar mapa de IDs remotos para merge
            const remoteIds = new Set(remoteAppointments.map(a => String(a.id)));
            const localIds = new Set(localAppointments.map(a => String(a.id)));
            
            // MERGE: Combinar dados locais e remotos
            const mergedAppointments = [];
            
            // 1. Adicionar todos os remotos (da planilha)
            ...
            // 2. Adicionar locais que NÃO estão na planilha (e enviar para planilha)
            const localOnly = localAppointments.filter(a => !remoteIds.has(String(a.id)));
            
            if (localOnly.length > 0) {
                console.log('📊 Encontrados', localOnly.length, 'agendamentos locais não sincronizados, enviando...');
                
                for (const apt of localOnly) {
                    // Adicionar ao merge
                    mergedAppointments.push(apt);
                    
                    // Enviar para planilha (silenciosamente)
                    try {
                        await this.createAppointmentSilent(apt);
                        console.log('📊 Enviado para planilha:', apt.clientName);
                    } catch (e) {
                        console.error('📊 Erro ao enviar:', e);
                    }
                }
            ...
            const oldData = JSON.stringify(localAppointments.map(a => `${a.date}-${a.time}-${a.clientName}`).sort());
            const newData = JSON.stringify(mergedAppointments.map(a => `${a.date}-${a.time}-${a.clientName}`).sort());
            
            if (oldData !== newData) {
                console.log('📊 Dados mudaram, salvando merge...');
                data.appointments = mergedAppointments;
                db.saveData(data);
    ...
    // Criar agendamento sem toast (usado pelo auto-sync)
    async createAppointmentSilent(appointment) {
        ...
        try {
            ...
            console.error('Erro ao criar (silent):', error);
            return { success: false, error: error.message };
        }
    }
}
